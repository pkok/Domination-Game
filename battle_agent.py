...
class Agent(object):
        ...
        if len(have_ammo) == 0:
            ...
            for id in jo.friends:
                if id not in assigned:
                    # Go to closest ammo
                    if ammos[0]["is_present"] and ammos[1]["is_present"]:
                        goals[id] = ammos[0]["location"] if (ammos[0]["distance"][id] < ammos[1]["distance"][id]) else ammos[1]["location"]
                    elif ammos[0]["is_present"]:
                        goals[id] = ammos[0]["location"]
                    elif ammos[1]["is_present"]:
                        goals[id] = ammos[1]["location"]
                    else:
                        am1_delay = max(ammos[0]["distance"][id], ammos[0]["timer"])
                        am2_delay = max(ammos[1]["distance"][id], ammos[1]["timer"])
    ...
    def get_action(self, randomize=False):
        ...
        if randomize:
            ...
            while random_distance > max_speed:
                random_distance = random.gauss(max_speed, 0.25 * max_speed)
            path = [(math.cos(random_angle) * random_distance,
                    math.sin(random_angle) * random_distance)]
        else:
    ...
    def finalize(self, interrupted=False):
        ...
        if self.id == 0 and hasattr(self, 'blobpath') and self.blobpath is not None:
            ...
            try:
                ...
                pickle.dump(data, blobfile, pickle.HIGHEST_PROTOCOL)
                print "Blob saved.\n"
            except:
...
def named_int(names, value=None):
    class NamedInt(int):
        def __init__(self, *args, **kwargs):
            int.__init__(self, *args, **kwargs)
            self._name = names[self % len(names)]
